...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SPI_Transmit(SPI_HandleTypeDef *hspi, const uint8_t *p_data, uint16_t Size, uint32_t Timeout)
{
  uint32_t tickstart;
  ...
  hspi->ErrorCode   = HAL_SPI_ERROR_NONE;
  hspi->pTxBuffPtr  = (const uint8_t *)p_data;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SPI_Receive(SPI_HandleTypeDef *hspi, uint8_t *p_data, uint16_t Size, uint32_t Timeout)
{
#if (USE_SPI_CRC != 0U)
  ...
  if ((hspi->Init.Mode == SPI_MODE_MASTER) && (hspi->Init.Direction == SPI_DIRECTION_2LINES))
  {
    ...
    return HAL_SPI_TransmitReceive(hspi, p_data, p_data, Size, Timeout);
  }
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SPI_Transmit_IT(SPI_HandleTypeDef *hspi, const uint8_t *p_data, uint16_t Size)
{
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SPI_Receive_IT(SPI_HandleTypeDef *hspi, uint8_t *p_data, uint16_t Size)
{
  ...
  if ((hspi->Init.Direction == SPI_DIRECTION_2LINES) && (hspi->Init.Mode == SPI_MODE_MASTER))
  {
    hspi->State = HAL_SPI_STATE_BUSY_RX;
    /* Call transmit-receive function to send Dummy data on Tx line and generate clock on CLK line */
    return HAL_SPI_TransmitReceive_IT(hspi, p_data, p_data, Size);
  }
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SPI_Transmit_DMA(SPI_HandleTypeDef *hspi, const uint8_t *p_data, uint16_t Size)
{
...
/**
  ...
  *               the configuration information for SPI module.
  * @param  p_data pointer to data buffer
  * @note   When the CRC feature is enabled the p_data Length must be Size + 1.
  * @param  Size amount of data to be sent
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_SPI_Receive_DMA(SPI_HandleTypeDef *hspi, uint8_t *p_data, uint16_t Size)
{
  /* Check rx dma handle */
  ...
  if ((hspi->Init.Direction == SPI_DIRECTION_2LINES) && (hspi->Init.Mode == SPI_MODE_MASTER))
  {
    ...
    /* Call transmit-receive function to send Dummy data on Tx line and generate clock on CLK line */
    return HAL_SPI_TransmitReceive_DMA(hspi, p_data, p_data, Size);
  }
